...
export class ExplorerDataSource implements IAsyncDataSource<ExplorerItem | ExplorerItem[], ExplorerItem> {
	...
	private resolveVirtualDirectories(elements: ExplorerItem[]) {
		...
		// TODO: Dynamic nested using config

		const possibleParrentElements = elements.filter(x => !x.isVirtualDirectoryMember && !x.isDirectory);

		let root = new Map<string, ExplorerItem>();
		possibleParrentElements.forEach(item => {
			let extDotIndex = item.name.indexOf('.');
			let ext = item.name.substr(extDotIndex + 1);
			let name = item.name.substr(0, extDotIndex);
		...
		possibleParrentElements.forEach(item => {
			...
			let parent = root.get(name);

			if (parent && item.name !== parent.name) {
				item.setVirtualDirectoryConfig('MEMBER', parent.name);
				parent.addVirtualChild(item);
			}
		});

		root.forEach((value: ExplorerItem) => {
			if (value.children.size > 0)
				value.setVirtualDirectoryConfig('DIRECTORY', value.name);
		});
